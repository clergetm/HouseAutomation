...
public class LightListenerImpl implements DeviceListener<GenericDevice>, OnOffConfiguration, AutoOnConfiguration {
	...
	// TODO REPLACE WITH LOCATION OF WAREHOUSE
	...
	public boolean leaveTheLightsOn(String location) {
		System.out.println(
				"[DAY] - The lights in the location : " + location + "stays on until the reset of the configuration.");
	...
	public void reset() {
		if (!this.isAutoMode) {
			System.out.println("[DAY] - Reset Auto On Configuration.");
			for (Entry<String, Integer> entry : this.passagesPerLocation.entrySet()) {
				// Reset value.
				entry.setValue(0);
				// Reset lights.
			...
			System.out.println("[DAY] - Reset Auto On Configuration cancelled by current activated auto mode On.");
			System.out.println("[DAY] - Please run 'turnlightsoff' command before resetting.");
		}
	}
